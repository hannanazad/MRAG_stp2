...
def extract_answer(captured: CapturedCall) -> tuple[str, str]:
    ...
    # Do not use arbitrary rendered Markdown as an answer fallback.
    # The notebook renders figure captions, retrieved chunks, scores, and
    # section headings as Markdown. When a provider returns an empty answer,
    # accepting one of those displays creates a silent false-success record.
    #
    # A substantive response must therefore come from:
    #   1. the explicit rendered "### Answer" section,
    #   2. the function return string/object, or
    #   3. a recognizable stdout answer section.
    #
    # Returning an empty string activates the existing retry mechanism.
    return "", "not_found"
